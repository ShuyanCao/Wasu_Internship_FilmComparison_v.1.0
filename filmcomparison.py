# Created and edited by Shuyan Cao
# shuyan.cao-1@student.uts.edu.au
...
df = pd.read_excel (r"E:\罗盘\资产相似度\ccms.xlsx", sheet_name = 'Sheet1', skipinitialspace = True)
df2 = pd.read_excel(r"E:\罗盘\资产相似度\douban.xlsx", sheet_name = 'Sheet1', skipinitialspace = True)
...
def seg_sentence(sentence):
    ...
    stopwords = stopwordslist(r'E:\罗盘\资产相似度\FilmComparison\FilmComparison\stopwords.txt')  # 这里加载停用词的路径
    outstr = []
...
def gensimcalculation(d1, d2):
    ...
    corpus = [dictionary.doc2bow(text) for text in texts]#重点
    tfidf = models.TfidfModel(corpus)#查一下
...
def main_function(): #重点        
    f1 = open("remain.txt", "w+")
    f2 = open("persons.txt", "w+")
    f3 = open("summary.txt", "w+")
    f4 = open("persons_summary.txt", "w+")
    f5 = open("title.txt", "w+")
    f6 = open("title_persons.txt", "w+")
    f7 = open("title_summary.txt", "w+")
    f8 = open("title_persons_summary.txt", "w+")
    dictremain = set()
    dictpersons = {}
    dictsummary = {}
    dictpersons_summary = {}
